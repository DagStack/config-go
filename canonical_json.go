...
// sorted in lexicographic UTF-8 code-point order.
//
...
func writeCanonicalObject(buf *bytes.Buffer, obj Tree) error {
	...
	// Lexicographic UTF-8 code-point order — Go's sort.Strings is
	// byte-order, which for valid UTF-8 coincides with code-point
	// order (UTF-8 preserves code-point ordering by design).
	sort.Strings(keys)
